...
public class HomeRecommendedFragment extends RxLazyFragment {
    ...
    RecyclerView mRecyclerView;//用于显示推荐页面的内容,包含:顶部的广告轮播,内容项列表(标题、两列视频项、内容栏底部)
    ...
    protected void initRecyclerView() {
        ...
        GridLayoutManager mLayoutManager = new GridLayoutManager(getActivity(), 2);//网格布局,分2列
        ...
        mRecyclerView.setLayoutManager(mLayoutManager);//设置网格布局
        ...
        RetrofitHelper.getBiliAppAPI()
                ...
                .flatMap(new Func1<List<RecommendBannerInfo.DataBean>, Observable<RecommendInfo>>() {
                    ...
                    public Observable<RecommendInfo> call(List<RecommendBannerInfo.DataBean> dataBeans) {
                        recommendBanners.addAll(dataBeans);//将getBiliAppAPI().getRecommendedBannerInfo请求结果保存到recommendBanners变量中
                ...
                .subscribe(resultBeans -> {
                    results.addAll(resultBeans);//将getBiliAppAPI().getRecommendedInfo请求结果保存到results变量中
                    finishTask();
    ...
    protected void finishTask() {
        ...
        hideEmptyView();
        convertBanner();
        mSectionedAdapter.addSection(new HomeRecommendBannerSection(banners));//添加轮播图Section
            ...
            if (!TextUtils.isEmpty(type)) {
                switch (type) {
                    ...
                    case ConstantUtil.TYPE_ACTIVITY_CENTER:
                        ...
                        break;
                    default://其他,例如:recommend(热门焦点),live(正在直播),region(动画区),region(舞蹈区)等
