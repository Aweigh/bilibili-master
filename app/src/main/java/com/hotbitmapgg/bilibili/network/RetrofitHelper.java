...
public class RetrofitHelper {
    ...
    public static BiliApiService getBiliAPI() {
        return createApi(BiliApiService.class, ApiConstants.ZZX_BASE_URL);//ApiConstants.API_BASE_URL);//使用ZZX服务器替换
    }

    public static BiliGoService getBiliGoAPI() {
        return createApi(BiliGoService.class, ApiConstants.ZZX_BASE_URL);//ApiConstants.BILI_GO_BASE_URL);//使用ZZX服务器替换
    }

    public static RankService getRankAPI() {
        return createApi(RankService.class, ApiConstants.ZZX_BASE_URL);//ApiConstants.RANK_BASE_URL);//使用ZZX服务器替换
    }

    public static UserService getUserAPI() {
        return createApi(UserService.class, ApiConstants.ZZX_BASE_URL);//ApiConstants.USER_BASE_URL);//使用ZZX服务器替换
    }

    public static VipService getVipAPI() {
        return createApi(VipService.class, ApiConstants.ZZX_BASE_URL);//ApiConstants.VIP_BASE_URL);//使用ZZX服务器替换
    }

    public static BangumiService getBangumiAPI() {
        return createApi(BangumiService.class, ApiConstants.ZZX_BASE_URL);//ApiConstants.BANGUMI_BASE_URL);//使用ZZX服务器替换
    }

    public static SearchService getSearchAPI() {
        return createApi(SearchService.class, ApiConstants.ZZX_BASE_URL);//ApiConstants.SEARCH_BASE_URL);//使用ZZX服务器替换
    }
    ...
    private static class CacheInterceptor implements Interceptor {
        @Override
        public Response intercept(Chain chain) throws IOException {
            // 有网络时 设置缓存超时时间1个小时( 60 * 60)
            int maxAge = 10;//60 * 60;//AW:这里将缓存时间修改短一点,测试时才会去请求新的数据。
            // 无网络时，设置超时为1天
